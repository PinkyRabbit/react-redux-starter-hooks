...
function getDevMiddleware(): ReadonlyArray<Middleware> {
  ...
  const { default: createImmutableStateInvariantMiddleware } = require('redux-immutable-state-invariant');
...
function hotReloadReducers(store: ReduxStore): void {
  if (isDevelopmentEnvironment && (module as any).hot) {
    (module as any).hot.accept('../modules/rootReducer', () => store.replaceReducer(rootReducer));
  }
}
